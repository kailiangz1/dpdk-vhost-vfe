...
{
	RTE_GLOBAL_FUNC_PTR_ERR_RET(pf_dev_vf_dev_prov, -ENOTSUP);

	return rte_rpc_vdpa_ops.pf_dev_vf_dev_prov(pf_name,
				vfid, vf_params);
}
			...
			struct vdpa_vf_params *vf_params)
{
	...
	return rte_rpc_vdpa_ops.pf_dev_vf_dev_add(pf_name,
				vfid, vf_params);
}
