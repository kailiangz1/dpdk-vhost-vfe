...
struct virtio_pci_internal vfe_virtio_pci_internal[VFE_MAX_PORT_NUM];
...
static int
legacy_setup_queue(struct virtio_hw *hw, struct virtqueue *vq, uint64_t avail_addr,uint64_t used_addr)
{
	uint32_t src;
...
const struct virtio_ops vfe_legacy_ops = {
...
static int
modern_setup_queue(struct virtio_hw *hw, struct virtqueue *vq ,uint64_t avail_addr,uint64_t used_addr)
{
	struct virtio_pci_dev *dev = virtio_pci_get_dev(hw);
	...
	rte_write16(vq->vq_queue_index, &dev->common_cfg->queue_select);
        /*TO_DO: queue size should set kailiang*/
...
const struct virtio_ops vfe_modern_ops = {
		...
		case VIRTIO_PCI_CAP_NOTIFY_CFG:
			...
			if (ret != 4)
				...
				dev->notify_base = get_cfg_addr(pci_dev, &cap);
                                //kailiang: add notify bar, we can use later
 ...
 */
int
vfe_vtpci_init(struct rte_pci_device *pci_dev, struct virtio_pci_dev *dev)
{
...
void vfe_vtpci_legacy_ioport_unmap(struct virtio_hw *hw)
{
	rte_pci_ioport_unmap(VTPCI_IO(hw));
}

int vfe_vtpci_legacy_ioport_map(struct virtio_hw *hw)
{
