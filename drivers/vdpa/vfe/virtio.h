...
#include <rte_ether.h>
/*VFE over VDPA max port supported*/
#define VFE_MAX_PORT_NUM 1024
...
struct virtio_ops {
	...
	int (*setup_queue)(struct virtio_hw *hw, struct virtqueue *vq,uint64_t avail_addr,uint64_t used_addr);
...
#define VIRTIO_OPS(hw)	(vfe_virtio_hw_internal[(hw)->port_id].virtio_ops)

extern struct virtio_hw_internal vfe_virtio_hw_internal[VFE_MAX_PORT_NUM];
...
uint64_t vfe_virtio_negotiate_features(struct virtio_hw *hw, uint64_t host_features);
uint8_t vfe_virtio_get_status(struct virtio_hw *hw);
void vfe_virtio_set_status(struct virtio_hw *hw, uint8_t status);
void vfe_virtio_write_dev_config(struct virtio_hw *hw, size_t offset, const void *src, int length);
void vfe_virtio_read_dev_config(struct virtio_hw *hw, size_t offset, void *dst, int length);
void vfe_virtio_reset(struct virtio_hw *hw);
void vfe_virtio_reinit_complete(struct virtio_hw *hw);
uint8_t vfe_virtio_get_isr(struct virtio_hw *hw);
#endif /* _VIRTIO_H_ */
