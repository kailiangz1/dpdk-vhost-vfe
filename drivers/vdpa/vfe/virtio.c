...
uint64_t
vfe_virtio_negotiate_features(struct virtio_hw *hw, uint64_t host_features)
{
	uint64_t features;
...
void
vfe_virtio_read_dev_config(struct virtio_hw *hw, size_t offset,
		      void *dst, int length)
{
...
void
vfe_virtio_write_dev_config(struct virtio_hw *hw, size_t offset,
...
void
vfe_virtio_reset(struct virtio_hw *hw)
{
	uint32_t retry = 0;
...
void
vfe_virtio_reinit_complete(struct virtio_hw *hw)
{
	vfe_virtio_set_status(hw, VIRTIO_CONFIG_STATUS_DRIVER_OK);
}

void
vfe_virtio_set_status(struct virtio_hw *hw, uint8_t status)
{
...
uint8_t
vfe_virtio_get_status(struct virtio_hw *hw)
{
	return VIRTIO_OPS(hw)->get_status(hw);
}

uint8_t
vfe_virtio_get_isr(struct virtio_hw *hw)
{
