		...
		struct virtio_admin_migration_identity_result *result)
{
	...
	res = (struct virtio_admin_migration_identity_result *)&ctrl->data[0];
...
static int
virtio_vdpa_cmd_set_status(struct virtio_hw *hw, uint16_t vdev_id,
...
static int
virtio_vdpa_cmd_resume(struct virtio_vdpa_pf_priv *priv, uint16_t vdev_id,
		enum virtio_internal_status status)
{
	struct virtio_hw *hw;
		...
		struct virtio_admin_migration_get_internal_state_pending_bytes_result *result)
{
	...
	res = (struct virtio_admin_migration_get_internal_state_pending_bytes_result *)(&ctrl->data[0] + dlen[0]);
		...
		struct virtio_admin_dirty_page_identity_result *result)
{
	...
	res = (struct virtio_admin_dirty_page_identity_result *)&ctrl->data[0];
		...
		struct virtio_admin_dirty_page_get_map_pending_bytes_result *result)
{
	...
	res = (struct virtio_admin_dirty_page_get_map_pending_bytes_result *)(&ctrl->data[0] + dlen[0]);
	result->pending_bytes = rte_le_to_cpu_64(res->pending_bytes);
...
RTE_INIT(virtio_vdpa_mi_init)
{
	struct virtio_vdpa_mi_ops mi_ops = {
		...
		.lm_cmd_get_internal_pending_bytes =
				virtio_vdpa_cmd_get_internal_pending_bytes,
		.lm_cmd_dirty_page_identity =
				virtio_vdpa_cmd_dirty_page_identity,
		.lm_cmd_dirty_page_start_track =
				virtio_vdpa_cmd_dirty_page_start_track,
		.lm_cmd_dirty_page_stop_track =
				virtio_vdpa_cmd_dirty_page_stop_track,
		.lm_cmd_dirty_page_get_map_pending_bytes =
				virtio_vdpa_cmd_dirty_page_get_map_pending_bytes,
		.lm_cmd_dirty_page_report_map =
				virtio_vdpa_cmd_dirty_page_report_map,
	};
