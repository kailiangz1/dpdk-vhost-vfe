		...
		struct vdpa_debug_vf_params *vf_debug_info)
{
	...
	switch(vf_debug_info->test_type) {
		case VDPA_DEBUG_CMD_GET_STATUS:
			//ret = virtio_vdpa_cmd_resume(vf_priv, vf_priv->vf_id, VIRTIO_S_RUNNING);
			ret = vf_debug_info->lm_status = 0;
			RPC_LOG(ERR, "vf debug: get lm status ret: %d", ret);
			break;
		case VDPA_DEBUG_CMD_RUNNING:
			ret = virtio_vdpa_cmd_resume(vf_priv->pf_priv, vf_priv->vf_id, VIRTIO_S_RUNNING);
			RPC_LOG(ERR, "vf debug: cmd_resume: VIRTIO_S_RUNNING, ret: %d", ret);
			break;
		case VDPA_DEBUG_CMD_QUIESCED:
			ret = virtio_vdpa_cmd_suspend(vf_priv->pf_priv, vf_priv->vf_id, VIRTIO_S_QUIESCED);
			RPC_LOG(ERR, "vf debug: cmd_suspend: VIRTIO_S_QUIESCED, ret: %d", ret);
			break;
		case VDPA_DEBUG_CMD_FREEZED:
			ret = virtio_vdpa_cmd_suspend(vf_priv->pf_priv, vf_priv->vf_id, VIRTIO_S_FREEZED);
			RPC_LOG(ERR, "vf debug: cmd_suspend: VIRTIO_S_FREEZED, ret: %d", ret);
			break;
