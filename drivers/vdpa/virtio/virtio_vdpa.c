...
virtio_vdpa_dev_close(int vid)
{
	...
	/* Suspend */
	ret = virtio_vdpa_cmd_suspend(priv->pf_priv, priv->vf_id, VIRTIO_S_QUIESCED);
	if (ret) {
	...
	ret = virtio_vdpa_cmd_suspend(priv->pf_priv, priv->vf_id, VIRTIO_S_FREEZED);
	if (ret) {
		DRV_LOG(ERR, "%s vfid %d failed suspend ret:%d", vdev->device->name, priv->vf_id, ret);
		return ret;
	}
...
virtio_vdpa_dev_config(int vid)
{
	...
	ret = virtio_vdpa_cmd_resume(priv->pf_priv, priv->vf_id, VIRTIO_S_QUIESCED);
	if (ret) {
	...
	ret = virtio_vdpa_cmd_resume(priv->pf_priv, priv->vf_id, VIRTIO_S_RUNNING);
	if (ret) {
