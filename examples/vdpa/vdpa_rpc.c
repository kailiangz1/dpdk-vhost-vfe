...
static cJSON *vdpa_vf_dev_add(const char *pf_name, struct vdpa_vf_info *vf_info)
{
	cJSON *result = cJSON_CreateObject();

	if (rte_vdpa_pf_dev_vf_dev_add(pf_name, vf_info)) {
		cJSON_AddStringToObject(result, "Error",
			"Fail to add VF device");
		return result;
	}
	if (rte_vdpa_get_vf_info(pf_name, vf_info->vfid, vf_info)) {
		cJSON_AddStringToObject(result, "Error",
		"Fail to find VF device");
		return result;
	}
	if (vdpa_with_socket_path_start(vf_info->vf_name)) {
		cJSON_AddStringToObject(result, "Error",
...
static void vdpa_vf_info_reformat(cJSON *device, struct vdpa_vf_info *vf_info)
{
	...
	uint8_t *addr_bytes = vf_info->mac.addr_bytes;

	JSON_STR_NUM_TO_OBJ(device, "vfid", "%u", vf_info->vfid);
	cJSON_AddStringToObject(device, "vf", vf_info->vf_name);
	JSON_STR_NUM_TO_OBJ(device, "msix_num", "%u", vf_info->msix_num);
	JSON_STR_NUM_TO_OBJ(device, "queue_num", "%u", vf_info->queue_num);
	JSON_STR_NUM_TO_OBJ(device, "queue_size", "%u", vf_info->queue_size);
	vdpa_rpc_get_info(device_feature,  vf_info->features,
			feature_names, VIRTNET_FEATURE_SZ);
	cJSON_AddItemToObject(device, "device_feature", device_feature);
	JSON_STR_NUM_TO_OBJ(device, "mtu", "%u", vf_info->mtu);
...
static cJSON *vdpa_vf_dev_list(const char *pf_name)
{
	...
	max_vf_num = rte_vdpa_get_vf_list(pf_name, vf_info, MAX_VDPA_SAMPLE_PORTS);
	if (max_vf_num <= 0) {
		cJSON_AddStringToObject(result, "Error",
			"Fail to dump all VF devices");
		rte_free(vf_info);
...
static cJSON *mgmtvf(jrpc_context *ctx, cJSON *params, cJSON *id)
{
	...
	if (vf_add && vf_id && pf_dev) {
		...
		/* Parse PCI device name*/
		vf_info.vfid = vf_id->valueint;
		vf_info.modify_flags = 0;
		if (msix_num) {
			vf_info.modify_flags |= (1 << VDPA_VF_MSIX_NUM);
			vf_info.msix_num = msix_num->valueint;
		}
		if (queue_num) {
			vf_info.modify_flags |= (1 << VDPA_VF_QUEUE_NUM);
			vf_info.queue_num = queue_num->valueint;
		}
		if (queue_size) {
			vf_info.modify_flags |= (1 << VDPA_VF_QUEUE_SZIE);
			vf_info.queue_size = queue_size->valueint;
		}
		if (features) {
			vf_info.modify_flags |= (1 << VDPA_VF_FEATURES);
			vf_info.features = features->valueint;
		}
		if (mtu) {
			vf_info.modify_flags |= (1 << VDPA_VF_MTU);
			vf_info.mtu = mtu->valueint;
		}
		if (mac) {
			vf_info.modify_flags |= (1 << VDPA_VF_MAC);
			sscanf(mac->valuestring,
			"%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
			&addr_bytes[0], &addr_bytes[1], &addr_bytes[2],
			&addr_bytes[3], &addr_bytes[4], &addr_bytes[5]);
		}
		result = vdpa_vf_dev_add(pf_dev->valuestring, &vf_info);
